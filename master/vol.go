...
func (vol *Vol) doCreateMetaPartition(c *Cluster, start, end uint64) (mp *MetaPartition, err error) {
	...
	if c.isFaultDomain(vol) {
		...
		zoneNum := c.decideZoneNum(vol.crossZone, proto.MediaType_Unspecified)
		if hosts, peers, err = c.getHostFromNormalZone(TypeMetaPartition, excludeZone, nil, nil, int(vol.mpReplicaNum), zoneNum, vol.zoneName, proto.StorageClass_Unspecified); err != nil {
